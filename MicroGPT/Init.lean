...
-- shapes, ids, and `requiresGrad` are wired correctly (wte is `vocab × nEmbed`, fc1 is `nEmbed × 4·nEmbed`)
#guard let (p, _) := initParams { nLayer := 1, nEmbed := 2, blockSize := 2, nHead := 1, vocabSize := 3, numSteps := 1 } { s := 123 }; p.wte.data.size == 6 && p.wte.id == ParamIds.wte && p.wte.requiresGrad && p.blocks.size == 1 && p.blocks[0]!.mlpFc1.data.size == 16
-- the LCG is deterministic in the seed: same seed reproduces, different seeds diverge
#guard arrApproxEq (initParams { nLayer := 1, nEmbed := 2, blockSize := 2, nHead := 1, vocabSize := 3, numSteps := 1 } { s := 123 }).1.wte.data (initParams { nLayer := 1, nEmbed := 2, blockSize := 2, nHead := 1, vocabSize := 3, numSteps := 1 } { s := 123 }).1.wte.data
#guard !arrApproxEq (initParams { nLayer := 1, nEmbed := 2, blockSize := 2, nHead := 1, vocabSize := 3, numSteps := 1 } { s := 1 }).1.wte.data (initParams { nLayer := 1, nEmbed := 2, blockSize := 2, nHead := 1, vocabSize := 3, numSteps := 1 } { s := 2 }).1.wte.data
